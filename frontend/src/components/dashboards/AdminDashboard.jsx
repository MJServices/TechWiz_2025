...
const AdminDashboard = () => {
  ...
  const handleAnnouncementAction = async (announcementId, action) => {
    try {
      if (action === 'toggle') {
        await announcementAPI.toggleStatus(announcementId);
  ...
  const handleAnnouncementSuccess = (newAnnouncement) => {
                    ...
                    <div className="bg-white/5 rounded-xl border border-white/10 overflow-hidden">
                        ...
                        <table className="w-full">
                          ...
                          <tbody className="divide-y divide-white/10">
                            {announcements.map((announcement) => (
                              <tr key={announcement._id} className="hover:bg-white/5 transition-colors">
                                <td className="px-6 py-4">
                                  <div>
                                    <p className="text-white font-medium">{announcement.title}</p>
                                    <p className="text-gray-400 text-sm line-clamp-2">{announcement.content.substring(0, 100)}...</p>
                                  </div>
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                  <div className="flex items-center space-x-2">
                                    <div className={`w-3 h-3 rounded-full ${
                                      announcement.priority === 5 ? 'bg-red-400' :
                                      announcement.priority === 4 ? 'bg-red-300' :
                                      announcement.priority === 3 ? 'bg-yellow-400' :
                                      announcement.priority === 2 ? 'bg-yellow-300' :
                                      'bg-green-400'
                                    }`}></div>
                                ...
                                <td className="px-6 py-4">
                                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                    announcement.isActive && (!announcement.expiresAt || new Date(announcement.expiresAt) > new Date())
                                      ? 'bg-green-500/20 text-green-400' 
                                      : 'bg-gray-500/20 text-gray-400'
                                  }`}>
                                    {announcement.isActive && (!announcement.expiresAt || new Date(announcement.expiresAt) > new Date()) ? 'Active' : 'Inactive'}
                                  </span>
                                </td>
                                <td className="px-6 py-4 text-gray-300">
                                ...
                                <td className="px-6 py-4">
                                  <div className="flex items-center space-x-2">
                                    <button
                                      onClick={() => handleAnnouncementAction(announcement._id, 'toggle')}
                                      className={`p-2 rounded-lg transition-colors ${
                                    ...
                                    >
                                      {announcement.isActive ? <Clock className="w-4 h-4" /> : <CheckCircle className="w-4 h-4" />}
                                    </button>
                                    <button
                                      onClick={() => handleAnnouncementAction(announcement._id, 'delete')}
                                      className="p-2 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors"
                      ...
                      {announcements.length === 0 && (
                        <div className="text-center py-12">
                          ...
                          <h3 className="text-xl font-medium text-gray-300 mb-2">No announcements yet</h3>
                          <p className="text-gray-400 mb-6">Create your first announcement to communicate with users</p>
                          <button
                            onClick={() => setShowCreateAnnouncementModal(true)}
