...
const EditEvent = () => {
  console.log('[DEBUG] EditEvent component mounted');
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      navigate('/login', { state: { returnTo: `/events/${id}/edit` } });
      return;
    }
  ...
  const fetchEvent = async () => {
    try {
      ...
      if (event.organizer !== user._id && user.role !== 'admin') {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('[DEBUG] Attempting to update event with data:', formData);
    try {
        ...
        <div className="relative z-10 container mx-auto px-4 py-8">
          ...
          <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 overflow-hidden hover:shadow-cyan-500/10 transition-all duration-300">
            <div className="p-6 md:p-8">
              <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent mb-8">Edit Event</h1>
